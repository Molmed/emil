...
##' @param surv.val When doing survival regression with a method that produce
...
batch.design <- function(type, x, y, param=NULL, ncv=10, subset=TRUE,
                         test.subset=FALSE, surv.val=1/3, assemble, 
        ...
        res.type <- foreach(my.type = type) %do% {
            tryCatch({
                ...
                if(!identical(my.ts, FALSE)){
                    pred <- predict(fit, x[!is.na(my.ts) & my.ts,, drop=FALSE])#, ...)
                    if(do.roc && !assemble)
                        pred <- roc.measures(y[!is.na(my.ts) & my.ts], pred)
                    if(is.outcome(y)){
                        pred$prob <- pred$pred
                        auto.pred <- predict(fit, x[design.idx,])
                        thres <- tune.survival(y$time[design.idx], y$event[design.idx], auto.pred$pred)
                        pred$pred <- factor(sapply(pred$prob, function(p) sum(p > thres)),
                                            levels=0:2, labels=c("low", "uncertain", "high"))
                    }
                } else {
                    pred <- NULL
                }
    ...
    if(assemble){
        try(res <- assemble.cv(res, y, subset=subset, test.subset=test.subset, verbose=verbose))
        if(do.roc){
